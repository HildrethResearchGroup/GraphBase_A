...
@interface DMTabBar() {
	...
	CGFloat kDMTabBarItemWidth;
    
}


//@property (readwrite) float kDMTabBarItemWidth;
...
@synthesize kDMTabBarItemWidth;

-(void) awakeFromNib {
	CGFloat barItemWidth = 32.0f;

	//kDMTabBarItemWidth = 32.0f;
	
	if ([[self delegate] respondsToSelector:@selector(dMTabBarWillSetTabBarItemWidth)]) {
		CGFloat barItemWidthFromDelegate = [[self delegate] dMTabBarWillSetTabBarItemWidth];
		if (barItemWidthFromDelegate > 0) {
			barItemWidth = barItemWidthFromDelegate;
		}
	}
	kDMTabBarItemWidth = barItemWidth;
}
